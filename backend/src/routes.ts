...
import { eventZodSchema, Profile, Event, EventInput, Log } from "./schema";
...
router.put("/event/:eventId", async (req, res) => {
  ...
  try {
    let oldEvent = await Event.findById(eventId).populate(
      "profileIds",
      "profileName"
    );
    if (!oldEvent) {
