...
export const StartDatePicker = ({
  ...
  const handleDateSelect = (date: Date | undefined) => {
    ...
    if (date.toDateString() === endDateTime.toDateString()) {
      ...
      if (newDateTime >= endDateTime) {
        newDateTime.setHours(endDateTime.getHours() - 1, endDateTime.getMinutes());
      }
  ...
  const getMaxTime = (): string => {
    if (startDateTime.toDateString() === endDateTime.toDateString()) {
      return formatTimeForInput(endDateTime);
    }
    return "23:59";
  };
    ...
    <div className="relative flex text-sm items-center gap-4">
      ...
      <div className="w-1/3 border-[0.5px] h-6 rounded">
        <input 
          ...
          style={{
            backgroundColor: startDateTime.toDateString() === endDateTime.toDateString() 
              ? '#f8f9fa' 
              : 'white'
          }}
          title={
            startDateTime.toDateString() === endDateTime.toDateString() 
              ? `Time must be before ${endDateTime.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}`
              : 'Select time'
          }
        />
      </div>
